...
int processo_com_prioridade(so_t *so){
    int menor_id = -1;
    int menor = 0;
    int quant_bloqueados = 0;
    int quant_prontos = 0;
    bool pegou_primeiro = false;

    for(int i = 0; i < so->qnt_programas; i++){
        if(so->processos[i].estado == PRONTO && so->processos[i].programa != NULL){
            if(!pegou_primeiro){
                menor = so->processos[i].tempo_cpu;
                menor_id = i;
                pegou_primeiro = true;
            }
            if(menor > so->processos[i].tempo_cpu){
                menor = so->processos[i].tempo_cpu;
                menor_id = i;
            }
        quant_prontos++;
...
int escalona(so_t *so){
    ...
    int id_processo_prioridade = processo_com_prioridade(so);
    if(id_processo_prioridade == -1){
        return -1;
    }else if(id_processo_prioridade == -2){
        return 0;
    }
    ...
    pid_executando = id_processo_prioridade;
    troca_estado(so->processos, id_processo_prioridade, PRONTO,EXECUCAO,tempo_relogio);
...
void interrompe_apos_despertar(so_t* so, int pid){
    int tempo_relogio = retorna_tempo(so->relogio);
