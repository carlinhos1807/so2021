...
processo_t *processo_cria()
{
    ...
    if(p != NULL){
        for(int i = 0; i < QNTPROCESSOS; i++){
            ...
            p[i].tempo_espera = 0;
            p[i].tempo_leitura = 20;
            p[i].tempo_escrita = 20;
...
so_t* so_cria()
{
    ...
    if(s != NULL){
        ...
        for(int i = 0; i < numero_quadros; i++){
            ...
            quadros[i].ultimo_tempo_utilizado = 0;
            quadros[i].id = i;
...
void copia(so_t *so, int idQuadro, int idPagina, int pid)
{
    ...
    pagina_seta_tempo(so->processos[pid].tabpag, idPagina, retorna_tempo(so->relogio));
...
void atualiza_tempos_quadros(so_t *so)
{
    for(int i =0; i < so->qnt_programas;i++){
        for(int j =0; j < numero_paginas;j++){
            int idQuadro = pagina_retorna_quadro(so->processos[i].tabpag, j);
            int idTempo = retorna_ultimo_uso_pagina(so->processos[i].tabpag, j);
            if(idQuadro != -1){
                so->quadros[idQuadro].ultimo_tempo_utilizado = idTempo;
            }
        }
...
int escolha_quadro(so_t *so)
{
    ...
    int menor = 0;
    int index = -1;
    bool pegou_primeiro_menor = false;
    if(quadro == -1){
        for(int i = 0; i < numero_quadros;i++){
            if(so->quadros[so->quadrosIds[i]].alterando == false){
                if(!pegou_primeiro_menor){
                    menor = so->quadros[so->quadrosIds[i]].ultimo_tempo_utilizado;
                    menorId = so->quadrosIds[i];
                    index = i;
                    pegou_primeiro_menor = true;
                }
                if(so->quadros[so->quadrosIds[i]].ultimo_tempo_utilizado < menor){
                    menor = so->quadros[so->quadrosIds[i]].ultimo_tempo_utilizado;
                    menorId = so->quadrosIds[i];
                    index = i;
                }
...
void desvincula_quadros(so_t *so,int pid)
{
    for(int i =0; i < numero_quadros;i++){
        if(so->quadros[i].idProcesso == pid){
            ...
            so->quadros[i].ultimo_tempo_utilizado = 0;
        }
    }
    return;
}
