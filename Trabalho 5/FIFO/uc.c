...
typedef enum {
  ...
  DESV,   DESVZ,  DESVNZ, LE,     ESCR,CRIA,   VALOR,  ESPACO, DEFINE,
} codigo_t;
    ...
    int* despertados = verifica_despertadores_desbloqueio(uc->relogio);
        ...
        if(despertados != NULL){
            ...
            for(int i = 0; i < quant_despertados; i++){
                if(cpu_estado_retorna_modo(retorna_processo_estado(uc->so, despertados[i])) == ERR_MMU_FAL_PAG){
                    interrompe_para_desbloquear(uc->so, despertados[i]);
                    int z = 0;
                    for(int n = i+1; n < quant_despertados;n++){
                        define_alarme_desbloqueio(uc->relogio, z+1, despertados[n]);
                        z++;
                    }
                    free(despertados);
                    return;
...
void executa(uc_t *uc)
{
    while (true) {
        ...
        int pid_executando = retorna_pid_executando(uc->so);
        altera_processo_status_executando(uc->so,pid_executando, true);

        err_t err = controla(uc, uc->cpu);
        if(err != ERR_CPU_PARADA_BLOQUEIO && err != ERR_MMU_FAL_PAG){
            altera_processo_status_executando(uc->so,pid_executando, false);
            disponibiliza_quadros(uc->so, pid_executando);
        }
...
err_t controla(uc_t *uc, cpu_t *cpu)
{
    ...
    int instrucao;
    err_t leituraResponse = mmu_le(retorna_mmu(cpu),PC,&instrucao);
    ...
    }else if(instrucao == CRIA){
        err_t erro = ERR_CPU_CRIA;
        cpu_altera_modo(cpu, erro);
        return ERR_CPU_CRIA;
    }else{
