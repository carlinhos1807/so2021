...
//intervalo de tempo para despertar, se <= 0, nao desperta
#define PERIODO_ESCALONAMENTO 2
...
void so_altera_relogio(so_t *so, relogio_t* relogio){
    so->relogio = relogio;
    define_alarme_periodico(relogio, PERIODO_ESCALONAMENTO);
    return;
}
void ajusta_nome_arquivo(int pid, char *vetor){

    char identificador_processo[100];
    itoa(pid, identificador_processo, 10);
...
void processo_libera(processo_t *processos){
    for(int i =0; i < QNTPROCESSOS; i++){
        //a memoria do ultimo processo executado ja foi liberada pela cpu
        if(i != pid_executando){
